...
// Image Buffers
var backBuf; // Main backbuffer from which screen is drawn
var backBufData; // Cache of backBuffer.getImage()
var earthTex; // Textured Earth land masses
...
Since JavaScript scripts cannot obtain true memory performance
parameters, these are paired on, and it is up to the user to adjust
them so that they work well in their actual system.

Okay, cache limits -- number of image textures, number of data array
entries that presumably take up so many bytes.

So since this is not possible to determine automatically, it is up to
the user to enter in this information manually and test with their own
system monitor.

...so when another process slows down the system, the idea is to break
down the JavaScript app peformance to compensate.  Then it is up the
the user/owner of the machine to remedy the problem.  There's a lot
that simply cannot be done reliably in JavaScript the necessity for
web browsers to run on average quality operating systems that can make
no guarantees about timeliness whatsoever.

Indicate when the server is slow.

cache size and data loading revolves primarily around setViewport()

render decides between generating new images for fetching from
existing images.  If a destination canvas is not given, then output
goes to frontBuf.  If frontBuf not allocated, then it is allocated.

front buffer? true/false

back buffers, zoom level

setViewport and render are preemptable cothread objects.

NOTE: AJAX is only asynchronous when your script is not doing
anything.  Otherwise, you have to use cothreads to provide
responsiveness.
